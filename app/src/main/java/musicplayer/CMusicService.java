...
public class CMusicService extends Service{
	...
	public static final String[] MODE_DESC = {"单曲播放", "列表播放", "随机播放", "顺序播放"};
	...
	private void notification(){
        ...
        rv.setTextViewText(R.id.tv_rv, "正在播放"+musicList.get(currentMusic).getTitle());  
	...
	public void notificationfun(){
		...
		notification = new Notification.Builder(this)
					.setTicker( "CrazyMusic正在播放"+musicList.get(currentMusic).getTitle())
	...
	private void playLast(){
		...
		case MODE_SEQUENCE:
			if(currentMusic - 1 == 0){
				Toast.makeText(this, "亲，已经到顶了！", Toast.LENGTH_SHORT).show();
			}else{
				play(currentMusic + 1, 0);
			}
			break;
